...
def run_single_registration(dataEntry):
    ...
    Parameters
    ----------
    dataEntry : a data entry given from a database for one stimulus (i.e, 'experiment') 
    for one mouse and one imaging session
    ...
    try:
        ...
        if len(dataEntry.Experiments[0]) > 1:
            # if concatinating across all stimuli before running suite2p, this will not create a stim subfolder for saving suite2p 
            saveDir = os.path.join(defs["rootDir"], dataEntry.Name, 'Processed', dataEntry.Date)
        else:
            saveDir = os.path.join(defs["rootDir"], dataEntry.Name, 'Processed', dataEntry.Date, dataEntry.Experiments[0]) 
        filePath = read_directory_dictionary(dataEntry, defs["rootDir"])
...
def z_register_one_file(ops):
   ...
   # TODO: modify this to include IBN datadirectory: animal/preprocessed/date/stimname/suite2p
