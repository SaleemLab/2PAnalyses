...
def get_nidaq_channels(niDaqFilePath, numChannels=None, plot=False):
    ...
    if numChannels is None:
        # this csv file saves the channels names corresponding to the nidaq bin file?
...
def get_piezo_data(ops):
    ...
    # Loads the frameclock from the nidaq. this is also found in the tif file 
    frameclock = nidaq[:, channels == "frameclock"]
    # Returns the time at which each frame was acquired.
    frames = assign_frame_time(frameclock, plot=True)
    # Loads the piezo.
