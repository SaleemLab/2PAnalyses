    ...
    for thisSession = 1:length(sessionNames)
        ...
        try
            ...
            for k = 1:length(stimList)
                ...
                elseif any(~contains(stimList{k}, vrKeywords, 'IgnoreCase', true))
            ...
            if ~isempty(otherVisualStim)
                ...
                for thisOtherVisualStim = 1:length(otherVisualStim)
                    ...
                    try
                        ...
                        [~, sessionFileInfo]       = getTuningStimEventsBonsaiFile(sessionFileInfo, otherVisualStimName, true); % overwrite = true
                        % Interpolate Suite2P, Bonsai and Peripheral Data to 60Hz
                        [~, ~, ~, sessionFileInfo] = ...
                            resamplAndAlignVisualStim_BonsaiPeripheralSuite2P(sessionFileInfo,interpRate,'TwoPFrameTime', otherVisualStimName, true, true, true);  % overwrite = true
                        % Delta F/F (Neuropil corrected)
                        [~, sessionFileInfo]       = computeNeuropilCorrectionAndDFF(sessionFileInfo, otherVisualStimName);
