...
%% Combine Adjacency Matrices (OR Logic)
% An ROI pair is considered correlated if it exceeds the threshold in BOTH VR OR NonVR.
adjMatrix_final = adjMatrix_VR | adjMatrix_NonVR;  %changed to OR 
adjMatrix_final(logical(eye(nROIs))) = false; % Remove self-correlation
fprintf('Combining adjacency matrices using OR logic (requiring **both** thresholds to be met).\n');
...
% output file for the sessionROIData.mat 
...
highlyCorrBoutons.adjMatrixWithOrLogic = adjMatrix_final; 
