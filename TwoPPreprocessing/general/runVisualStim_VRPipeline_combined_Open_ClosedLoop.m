...
filteredTable = filterMasterTable('Exclude', 0, ...
    ...
    'MouseID', {'M26003',}, ... 
    'Session', {'20260421'}); %rerun '20260318' m25131 (checking eye tracking bits here); m26003 20260321 (too)
mouseInfo = sessionsToProcess(filteredTable);
% newOrder = [3, 1, 2, 4, 5];
responseName = {'BaselineCorridor_OpenLoop','BaselineCorridor_OpenLoop', 'BaselineCorridor_OpenLoop', 'LandManipCorridor','LandManipCorridor','LandManipCorridor','LandManipCorridor'};
% 
...
paramsSomas.useZScoredProcessedSignals = false; emporalSmoothing = true;
...
for thisMouse = 1:size(mouseInfo, 1)
    ...
    for thisSession = 1:length(sessionNames)
        ...
        try
            ...
            %                 [response, sessionFileInfo] = getTrialResponsePSTH(sessionFileInfo, thisName, signalName);
            %                 plotSpeedTuning(sessionFileInfo, response, true)
            % 
            ...
            %                 plotDirectionTuning(sessionFileInfo, response,false, false)
            ...
            if ~isempty(vrStimNames)
                ...
                % Split files into those to combine (OpenLoop) and those to process individually
                idxToCombine = contains(vrStimNames, 'OpenLoop', 'IgnoreCase', true);
                combineList = vrStimNames(idxToCombine);
                individualList = vrStimNames(~idxToCombine);

                % --- PATH 1: COMBINED (OpenLoop Files) ---
                if length(combineList) > 1
                    ...
                    responseVRRuns = loadDataStructuresByKeyword(sessionFileInfo, 'OpenLoop', 'Response');
                    ...
                    plotPopulationActivityAcrossConditions(sessionFileInfo, response, 'dFF', applyTemporalSmoothing);
                    ...
                    [response, sessionFileInfo] = computeShuffleMatrixForSession(sessionFileInfo, response, combineList, useZScoredProcessedSignals);
                    ...
                    plotPopulationActivityAcrossConditions_HalvesStableROIsOnly(sessionFileInfo, response, 'dFF', applyTemporalSmoothing);
                ...
                % --- PATH 2: INDIVIDUAL (Non-OpenLoop Files; closed-loop) ---
                if ~isempty(individualList)
                    fprintf('Processing via INDIVIDUAL LOOP path.\n');
                    for i = 1:length(individualList)
                        vrStimName = individualList{i};
                        ...
                        [~,~,~] = checkHalvesCorrelation(sessionFileInfo, response);
                         ...
                         plotPopulationActivityAcrossConditions_HalvesStableROIsOnly(sessionFileInfo, response, 'dFF', applyTemporalSmoothing);
                         plotPopulationActivityAcrossConditions_HalvesStableROIsOnly(sessionFileInfo, response, 'dFFNeuropilCorrected', applyTemporalSmoothing);
                         plotPopulationActivityAcrossConditions_HalvesStableROIsOnly(sessionFileInfo, response, 'spks', applyTemporalSmoothing);
                         plotAllNeuronConditionsSummaries_VR_and_RF(sessionFileInfo, response, applyTemporalSmoothing, 'dFFNeuropilCorrected');                       
                         % For boutons specifically
                        if strcmpi(typeImaged, 'Boutons')
                            disp('Finding highly correlated boutons for this session')
