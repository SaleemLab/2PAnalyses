function [bonsaiData, sessionFileInfo] = findBonsaiPeripheralLag(sessionFileInfo, method, samplingRate, StimName)
...
for iStim = 1:length(sessionFileInfo.stimFiles)
    bonsaiData.isVRstim(iStim) = strcmp(StimName,sessionFileInfo.stimFiles(iStim).name);
end
...
if exist(sessionFileInfo.stimFiles(iStim).BonsaiData, 'file') && ...
   exist(sessionFileInfo.stimFiles(iStim).processedPeripheralData, 'file')
    ...
    error('Missing Peripheral or Bonsai data for stimulus "%s".', stimName);
end
...
%         % Correct ArduinoTime for selected variables / changed to a
%         different function.
%         % ---- Peripherals ----
%         peripheralData.Quadstate.lagCorrArdinoTime = rawQuadArduinoTime-(bestLag*(1/sampling_rate));
%         peripheralData.Wheel.lagCorrArdinoTime = peripheralData.Wheel.rawArduinoTime-(bestLag*(1/sampling_rate));
%         
%         % ---- Bonsai VR ----
%         bonsaiData.MousePos.lagCorrArduinoTime = bonsaiData.MousePos.rawArduinoTime-(bestLag*(1/sampling_rate));
%         bonsaiData.TrialInfo.lagCorrArduinoTime = bonsaiData.TrialInfo.rawArduinoTime-(bestLag*(1/sampling_rate));
