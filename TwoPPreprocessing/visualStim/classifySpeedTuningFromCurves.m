...
% CLASSIFYSPEEDTUNINGFROMCURVES_V3
%
% Multi-Template Competitive Gaussian Fit — Edd-H, tightened bounds.
%
% 
%   • Trust the residual competition — the best-fitting template wins.
...
%   • R2 and p_shuffle thresholds applied in population script, not here
    ...
    for f = 1:numel(frameworks)
        ...
        cls.classifierVersion = "gaussCompete_linearSpace_v4";
        ...
        fprintf('Running Edd-style v3 final fits on %d ROIs [%s]...\n', numROIs, targetStruct);
        ...
        for r = 1:numROIs
            ...
            while idx_idx <= 6
                ...
                if bestIdx == 1 || bestIdx == 2
                    ...
                    % Reject if minimum is interior AND right-side rise > 15% of range
                    % Tightened from 0.25 to catch more trough-shaped cells
                    [~, minidx_lp] = min(yvals);
                    notATrough = ~(minidx_lp > 1 && minidx_lp < numel(yvals) && ...
                                   (yvals(end) - yvals(minidx_lp)) > 0.15 * rangeVal);
                    if firstBinWasFit && (maxidx == 1) && noSecondaryPeak && notATrough
                ...
                elseif bestIdx == 3 || bestIdx == 4
                    % Data must rise substantially from first to last bin
                    strongRise = (yvals(end) - yvals(1)) > 0.30 * rangeVal;
                    if lastBinWasFit && (yvals(end) >= 0.80 * maxVal) && strongRise
                        bestParams = candidateParams;
                ...
                elseif bestIdx == 5
                    ...
                    [~, ~, ~, prom] = findpeaks(fitY);
                    % Sigma must span at least 1 bin width — rejects spike artefacts
                    sigmaOK = candidateParams(4) >= mean(diff(xvals));
                    if ~isempty(prom) && prom(1) >= rangeVal / 3 && sigmaOK
                        bestParams = candidateParams;
    ...
    fprintf('v3 final classifier finished.\n');
end
