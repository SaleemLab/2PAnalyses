...
%      FRACTION (degenerateSigmaFrac, default 0.5) of the actual tested span (range(azRange) /
%      range(elRange)) on that axis? The fitting bound (ub(4)/ub(5) = 3*range) is deliberately
%      loose so the optimizer never hits a wall -- it is NOT a claim about plausible RF size, and
%      a fit can sit nowhere near that bound while still being unidentifiable. Visual inspection
%      of fits with sigma at ~45-60% of the tested range showed clear monotonic, edge-going
%      responses along that axis -- never turning over anywhere in the grid -- confirming that
%      even sigma well below the FULL range can be unidentifiable: seeing a Gaussian turn over on
%      both sides of its peak requires roughly 2-3 sigma-widths of coverage, not just 1. This
%      check GATES isTrusted alongside the R^2/bootstrap-CI criteria (previously this exclusion
%      was accidentally left commented out in the commit block -- FIXED here; without it, the
%      trusted count was 114 instead of the validated 82, silently re-including the
%      monotonic/degenerate fits this check exists to catch).
%
...
% drawn off-screen.
screenAzLimits = [-80, 30];   % NaN = no clipping on this side (extends past the stimulus footprint)
...
%     degenerateSigmaFrac = 0.45; % sigma must be no more than this fraction of the tested range to
%                                 % be considered resolvable; select based on the sensitivity sweep
%     isDegenerateSigmaAz = pFit(4) >= degenerateSigmaFrac * range(azRange);
%     isDegenerateSigmaEl = pFit(5) >= degenerateSigmaFrac * range(elRange);
%     isDegenerateSigma   = isDegenerateSigmaAz || isDegenerateSigmaEl;

    degenerateSigmaFracAz = 0.45;
    degenerateSigmaFracEl = 0.65; % looser than az -- only 3 elevation grid positions
    isDegenerateSigmaAz = pFit(4) >= degenerateSigmaFracAz * range(azRange);
    isDegenerateSigmaEl = pFit(5) >= degenerateSigmaFracEl * range(elRange);
    isDegenerateSigma   = isDegenerateSigmaAz || isDegenerateSigmaEl; % too large on either axes
