...
for i = 1:numel(thal_idx)
    iunit = thal_idx(i);
    shadedErrorBar(1:6,...
        cellfun(@(x) mean(x,2), units(iunit).allSpikes(:,1)),...
        cellfun(@(x) sem(x,2), units(iunit).allSpikes(:,1)))

        shadedErrorBar(1:6,...
        cellfun(@(x) mean(x,2), units(iunit).allSpikes(:,2)),...
        cellfun(@(x) sem(x,2), units(iunit).allSpikes(:,2)),...
        'lineProps','r')
