...
% Use the processed signals (not zsoredprocessed) and the frames times for
% each trial window and extract data to save. Data is arranged by stim time
% and the pre-post windows are defined and relative to stim onset at time
% 0s.
%
% NOTE: this function does NOT baseline-subtract. alignedResponses is the
% raw interpolated trace only. Baseline subtraction is left to whichever
% downstream analysis uses this data, since different analyses may want
% different baseline windows (e.g. analyseRFMapping.m uses [-1, 0]).
%
% Both an interpolated (nearest, no extrapolation) time-resolved matrix
% AND the completely raw, non-interpolated per-trial data are stored, so
% downstream code can use whichever it needs.
...
% Initialize 3D matrix with the Perfect Grid size
    ...
    trialRawTimes  = cell(nTrialsInGrp, 1); % raw, un-resampled relative times per trial
    trialRawSignal = cell(nTrialsInGrp, 1); % raw signal at those times, [nNeurons x nRealSamples]
...
if response(1).badTrialMask(trialID), continue; end
        ...
        % store the raw, non-interpolated trace directly (no resampling)
        trialRawTimes{ti}  = rawRelTimes(valid);
        trialRawSignal{ti} = rawSignal(:, valid);

% interpolate (nearest, no extrap) for the time-resolved matrix
for n = 1:nNeurons
if sum(valid) > 10
                trace = interp1(rawRelTimes(valid), rawSignal(n, valid), Time, 'nearest');
                aligned(n, :, ti) = trace;
end
end
end
...
% Store
    ...
    response(1).psthData(g).responseType     = 'alignedResponses :Nearest-sample aligned, no baseline-subtraction, skipped bad trials (for the raw version too)';
    response(1).psthData(g).trialRawTimes    = trialRawTimes;
    response(1).psthData(g).trialRawSignal   = trialRawSignal;
end
save(sessionFileInfo.stimFiles(iStim).Response, 'response');
fprintf('PSTH complete (interpolated + raw) for %s.\n', stimName);
end
