...
% method below active. DSI/prefDirectionDeg are left as all-NaN so
% downstream code that checks for these fields doesn't error, it just
% won't have vector-method values. NOTE: prefDirectionDeg (0-360 deg,
% vector method) is used by plotExampleDirTuningBouton.m's annotation
% text -- swap that to a simple-ratio-derived preferred direction if you
% want the plot annotation to keep working without the vector method.
...
prefDirectionDeg_simple = nan(nBoutonsTotal, 1); % preferred direction from the simple-ratio method (Rpref's angle), since vector-method prefDirectionDeg is no longer computed
...
for b = 1:nBoutonsTotal
    ...
    prefDirectionDeg_simple(b) = thetaSorted(prefIdx);
end

%% ===================== attach results =====================
for b = 1:nBoutonsTotal
    allDirTuning(b).DSI                     = DSI(b);
    allDirTuning(b).prefDirectionDeg        = prefDirectionDeg(b);        % vector method -- all-NaN now, kept for compatibility
    allDirTuning(b).prefDirectionDeg_simple = prefDirectionDeg_simple(b); % simple-ratio method -- use this instead
...
if makePlots
    ...
    title(sprintf('Simple-ratio DSI (n=%d, criterion: %s)', nValidSimple, responsivenessField), ...
        'Interpreter', 'none');

    figure;
    polarhistogram(deg2rad(prefDirectionDeg_simple(~isnan(prefDirectionDeg_simple))), 16); % NOT doubled -- direction spans full 360
    title('Preferred direction distribution (simple-ratio method)');

    % vector-method plots -- commented out along with the vector method
    % itself, above
