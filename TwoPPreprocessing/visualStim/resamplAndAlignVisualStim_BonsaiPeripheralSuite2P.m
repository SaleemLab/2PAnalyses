...
%%frames mask to used later on..frames labelled as 'bad' are turned into
%%nans while extracting twop frames into descritised position bins
%%(lapPositionActivity) 
...
%   - Includes bad frames from ops.(these are f
...
raw2PTimes = vertcat(twoPData.(mainTimeToUse)); % concat twoP frame times from all planes 
[unique2PTimes, mainTimeUniqueIdx] = unique(raw2PTimes); % this likely not happen 
...
processedTwoPData.redcell = []; % this will most likely be empty except for a handful of RSP bouton recordings 
...
%     end
% end
%% GrayScreen Check: Remove fields if necessary; this is for when the gray screen recording used the vr script (pre 2025 sessions) 
...
% As pupil time stamps are logged on a different computer and arduino; 
disp('Processing Peripheral Data: Pupil')
if isfield(peripheralData, 'Pupil') && ~isempty(peripheralData.Pupil.raw.Area) && ~isempty(peripheralData.Pupil.raw.ArduinoTime) 
