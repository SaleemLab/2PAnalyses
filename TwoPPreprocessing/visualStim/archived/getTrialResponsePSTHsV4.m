...
% signalMatrix = [];
% planeIdx      = [];
% %for p = 1:numel(processedTwoPData)
% 
% F    = processedTwoPData.F;       % [nROI x nFrames]
% Fneu = processedTwoPData.Fneu;    % [nROI x nFrames]
...
function [response, sessionFileInfo] = getTrialResponsePSTHsV4( ...
    sessionFileInfo, stimName, method, interpRate)
    ...
    for ti = 1:nTrials
        ...
        for ni = 1:nNeurons
            ...
            if ~all(isnan(trialData(ni,:)))
                aligned(ni,:,ti) = interp1(relTimes, trialData(ni,:), timeVector, 'linear', NaN);
            end
        end
    end
    
    % Compute stats (ignoring NaNs from trials that might be outside time bounds)
    mResp = nanmean(aligned, 3);
    sResp = nanstd(aligned, 0, 3);
    semR  = sResp ./ sqrt(sum(~isnan(aligned), 3));
