...
% From processed2Pdata
if useZScoredProcessedSignals
    % This only loads the z-scored struct variable from the root of the file
    load(sessionFileInfo.stimFiles(stimIdx).processedMergedBonsaiSuite2pData, 'zScoredProcessedSignals', 'iscell');
    signals = zScoredProcessedSignals;
else
    load(sessionFileInfo.stimFiles(stimIdx).processedMergedBonsaiSuite2pData, 'processedSignals', 'iscell');
...
response.signalsZScored = useZScoredProcessedSignals;
