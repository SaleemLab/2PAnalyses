...
% Dynamic integration of spks if available in shuffle matrix
if isfield(response.lapPositionActivity_ShuffleMatrix, 'spks')
...
for iSignal = 1:length(signalNames)
    ...
    % Safety guard if a signal is in the list but missing from real lap activity
    if ~isfield(response.lapPositionActivity, currentSignalName)
        warning('Field %s not found in lapPositionActivity. Skipping.', currentSignalName);
        continue;
    end
    ...
    meanRealTuningCurve = mean(realActivity, 2, 'omitnan'); % (numROIs x 1 x numBins)
    meanRealTuningCurve = squeeze(meanRealTuningCurve); % (numROIs x numBins)
    
    % Handle single-ROI squeeze edge case (ensures matrix shape holds)
    ...
    % Find the peak value for each shuffle, for each neuron
    ...
    % Handle single-ROI squeeze edge case for shuffle array
    if numROIs == 1 && size(shufflePeakValues, 1) ~= 1
        shufflePeakValues = shufflePeakValues';
    end
    ...
    for thisROI = 1:numROIs
        % Distribution of peak values for the current neuron across all shuffles
    ...
    % Store the results in the local structs
