...
% 'gaussianR2'    = 2D Gaussian fit R^2 > r2Thresh, fit on all
%   boutons that pass ANOVA alone (independent of the amplitude gate).
% NOTE: the ANOVA alpha (0.05) and SD threshold (2) themselves are
% set inside analyseRFMapping.m, not here -- edit that function if
% you want to change the upstream isResponsive definition itself.
crossValVarName = 'crossValExpVar';   
%  update to match the actual variable name in
% sessionROIData for the cross-validated explained variance
% from your other (e.g. moving-dot tuning) stimulus. If this
% guess is wrong, the script will print the available variable
% names in sessionROIData the first time it's loaded so you can
...
allMedianCrossValR2  = [];  % cross-validated explained variance from the other stimulus, per bouton
allMeanCrossValR2    = [];  % FIX: Initialized missing array for mean cross-val R2
allShuffPVal         = [];  % FIX: Initialized missing array for shuffled p-values
        ...
        if isfield(vrSessionData, crossValVarName) && ~isempty(vrSessionData.(crossValVarName)) ...
            ...
            if isfield(sigData, 'meanExpVar')
                ...
                if numel(candidateMeanExpVar) == numel(thisRFMapping)
                    ...
                    % FIX: Changed warning text from 'pValues length' to 'meanExpVar length' for accuracy
...
for iROI = 1:numBoutons
    ...
    % critera 
    isResponsive = (pValANOVA < 0.05)   && (prefVal > (blankMean + 1.5 * blankStd));
...
responsiveBoutons = [2330, 3182, 3565];   %  3189, 
...
%%  PANEL C 
% Side-by-side histograms of preferred Az / El, plus Az-vs-El scatter
% Extract preferred coordinates for responsive boutons only
isResp = [allRFMapping.isResponsive];
responsiveBoutons = allRFMapping(isResp);

prefAz = [responsiveBoutons.centerAz];
prefEl = [responsiveBoutons.centerEl];

% Filter out any leftover NaNs just to be clean
validCoords = ~isnan(prefAz) & ~isnan(prefEl);
prefAz = prefAz(validCoords);
prefEl = prefEl(validCoords);


%% 
figure('Position', [100, 100, 850, 280]);
% az histogram
subplot(1,3,1);
if length(uAz) > 1
    azStep = mean(diff(uAz));
    azEdges = (min(uAz) - azStep/2) : azStep : (max(uAz) + azStep/2);
    histogram(prefAz, azEdges, 'Normalization', 'probability', 'FaceColor', [0.3 0.5 0.8]);
else
    histogram(prefAz, 'Normalization', 'probability', 'FaceColor', [0.3 0.5 0.8]);
end
...
if length(uEl_plot) > 1
    elStep = abs(mean(diff(uEl_plot))); % use abs in case it's descending
    elEdges = (min(uEl_plot) - elStep/2) : elStep : (max(uEl_plot) + elStep/2);
    histogram(prefEl, elEdges, 'Normalization', 'probability', 'FaceColor', [0.8 0.4 0.3]);
else
    histogram(prefEl, 'Normalization', 'probability', 'FaceColor', [0.8 0.4 0.3]);
end
...
% write the count directly on each bubble instead of a separate legend --
% avoids wasting figure space and guarantees the number always matches
% that exact bubble's size, unlike a legend with a few sampled sizes.
hold on;
...
% Quick check: is RF-mapping responsiveness related to how well-explained a
...
if any(~isnan(allMedianCrossValR2))
    ...
    hold on;
    % use shared, explicit bin edges so both groups are directly comparable
    edgesShared = linspace(-0.6, 1, 40);  % adjust range/count to your data

    histogram(allMedianCrossValR2(candidateRespIdxList), edgesShared, 'Normalization', 'probability', ...
    'FaceColor', [0.2 0.6 0.2], 'FaceAlpha', 0.6, 'DisplayName', 'RF-responsive');
    histogram(allMedianCrossValR2(candidateUnrespIdxList), edgesShared, 'Normalization', 'probability', ...
    'FaceColor', [0.6 0.2 0.2], 'FaceAlpha', 0.6, 'DisplayName', 'RF-non-responsive');
    xlabel('Cross-validated R^2 (other stimulus)', 'FontName', 'Arial', 'FontSize', 9);
    ylabel('Proportion of boutons', 'FontName', 'Arial', 'FontSize', 9);
    legend('Location', 'best');
    set(gca, 'Box', 'off', 'TickDir', 'out', 'FontName', 'Arial', 'FontSize', 8);
    title('Do RF-responsive boutons also have better cross-val tuning elsewhere?', ...
        'FontName', 'Arial', 'FontSize', 9, 'FontWeight', 'normal');
else
