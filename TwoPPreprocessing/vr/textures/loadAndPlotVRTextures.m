...
%% testing gemini's version with the LUT
%%---------------------------PARAMETERS---------------------------------
corridorL = 200; 
corridorH = 12; 
texwidth = 0.04; 
...
%%---------------------------LOAD LUT CALIBRATION-----------------------
% Load the BMP LUT file you uploaded
lutImg = imread(fullfile(basePath, 'NoRed_New_HALFINTENSITY_LUT_SALEEM20_20260213.bmp'));

% Convert to double [0, 1] range
lutImg = double(lutImg) / 255;

% Extract a single 1D profile. Since it's a grayscale-to-cyan gradient, 
% we can average across the active green and blue channels to get the luminance profile.
lut_profile = squeeze(mean(lutImg(1, :, 2:3), 3)); 
lut_x = linspace(0, 1, length(lut_profile));
...
for k = 1:4
    ...
    % --- APPLY THE LUT OVERLAY ---
    % Map the texture's grayscale values through your experimental LUT curve
    img = interp1(lut_x, lut_profile, img, 'linear');
