function plotAllNeuronSummariesToPDF_withThresholds(sessionFileInfo, response, applySmoothing)
%   Plots summary for all ROIs (cells and non-cells) and exports all
%   plots into one multipage PDF (one page per ROI). Includes threshold checks.
%
% Inputs:
...
pdfName = sprintf('%s_%s_AllROIs_%s_dFFNeuropilCorrected_MeansAndLapPositionHeatMap_THRESHOLDED.pdf', ...
...
for neuronIdx = 1:nROIs
    ...
    if metricsLoaded
        % Get metrics for the current ROI
        isPeakSig = peakSigIndex(neuronIdx);
        isRangeSig = rangeSigIndex(neuronIdx);
        peakRank = peakRankVal(neuronIdx); 
        rangeRank = rangeRankVal(neuronIdx); 
        ...
        if passesAllCriteria
            ...
            statusStr = 'All critera met';
        else
            titleColor = [0.8 0 0]; % Red
            statusStr = 'Not all critera met';
        end
        
        % Detailed text box content - Displaying RANK value with pass/fail status in parentheses
        detailText = {
            sprintf('Peak Rank (95%%): %.1f%% (%d)', peakRank, isPeakSig), 
        ...
        % Default setup if metrics couldn't be loaded
    ...
    % Add the detailed text box (only if metrics loaded)
    if metricsLoaded
        dim = [0.15 0.5 0.2 0.4]; % Position [x y width height]
...
if metricsLoaded && nROIs > 0
    % Exclude ROIs that were skipped due to all NaNs (should be minimal)
    ...
    fprintf('OVERALL CELL SUMMARY:\n');
    fprintf('ROIs passing ALL four criteria (Sig, Quality, Range, Kept): %d / %d\n', totalROIsPassingAll, validROIs);
