function plotPopulationActivityAcrossConditions(sessionFileInfo, response, signalToUse, applySmoothing)
% plotPopulationActivityAcrossConditions 
...
filename = fullfile(figSaveDir, sprintf('%s_%s_%s_%s_Pop_Conditions_Summary.png', ...
...
fig = figure('Color', 'w', 'Position', [50 100 350*nPlots 400], 'Visible', 'on');
...
for i = 1:nPlots
    ...
    for lIdx = 1:4
        ...
        % If it's a manipulation trial, check if the digit (1-4) is in the name
    ...
    if i == 1
        ylabel('ROIs (sorted by odd baseline)'); yticks([1 nROIs]);
    else
        set(gca, 'YTickLabel', []);
    end
...
header = sprintf('%s | %s | %s ', sessionFileInfo.animal_name, sessionFileInfo.session_name, signalToUse);
