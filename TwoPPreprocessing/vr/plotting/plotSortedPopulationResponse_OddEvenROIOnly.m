function plotSortedPopulationResponse_OddEvenROIOnly(sessionFileInfo, response, processedTwoPData, applySmoothing)
%   Plots normalised population response heatmaps (odd vs even laps),
%   sorted by peak response location in odd laps.
%   Only includes ROIs specified in response.cellROIs.
%
% Inputs:
%   sessionFileInfo : struct
%   response : struct
%       Must include:
%           - response.lapPositionActivity (neurons x laps x bins)
%           - response.signalUsed
%           - response.cellROIs (indices of ROIs to include)
...
lapActivity = lapActivityFull(roiIdx, :, :);
