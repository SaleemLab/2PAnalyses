function figHandle = plotPooledPopulation_OddEvenWithExampleMapped(allData, targetArea, varargin)
...
%
% NEW: pass example (session, ROI) pairs via 'ExampleSessions' /
% 'ExampleROIs' to mark their rows in the sorted heatmap with numbered
% arrows (1, 2, 3, ...). Marker rows are computed from the Odd sort order
% and reused for both Odd and Even panels.
    ...
    % --- NEW: example ROI highlighting ---
    % ExampleSessions / ExampleROIs must be same-length vectors, e.g.:
    %   'ExampleSessions', [15, 21, 21, 15], 'ExampleROIs', [11, 71, 65, 2]
    % Row 1 of the vectors gets marker "1", row 2 gets "2", etc.
    addParameter(p, 'ExampleSessions', [], @isnumeric);
    addParameter(p, 'ExampleROIs', [], @isnumeric);

    parse(p, allData, targetArea, varargin{:});

    targetFont = p.Results.FontName;
    exampleSessionsIn = p.Results.ExampleSessions;
    exampleROIsIn     = p.Results.ExampleROIs;
    ...
    for d = 1:nDays
        ...
        allOdd = []; allEven = [];
        cellSessionID = []; cellROIid = [];   % --- NEW: track (session, ROI) identity per row ---
        ...
        for s = 1:length(daySessions)
            ...
            % --- NEW: record which (session, ROI) each row corresponds to.
            % IMPORTANT: replace 'thisSessionID' below with whatever field
            % actually identifies the session in your indexing scheme (the
            % same numbering you use in plotRoiSpatialTuning(dataToUse, 15, 11, ...)).
            % Common candidates: thisSess.SessionID, thisSess.SessionNum,
            % thisSess.Session, or the loop index s if daySessions order
            % matches that numbering. Using 's' as a placeholder for now.
            thisSessionID = s;   % <-- ADJUST THIS LINE
            cellSessionID = vertcat(cellSessionID, repmat(thisSessionID, length(idx), 1));
            cellROIid     = vertcat(cellROIid, idx(:));
        ...
        % --- NEW: reorder identity vectors to match the sorted matrix ---
        sortedSessionID = cellSessionID(sIdx);
        sortedROIid     = cellROIid(sIdx);

        % --- NEW: find the sorted row for each requested example ---
        exampleRows = [];
        if ~isempty(exampleSessionsIn)
            exampleRows = nan(1, length(exampleSessionsIn));
            for e = 1:length(exampleSessionsIn)
                match = find(sortedSessionID == exampleSessionsIn(e) & ...
                             sortedROIid == exampleROIsIn(e), 1);
                if ~isempty(match)
                    exampleRows(e) = match;
                end
            end
        ...
        plotQueue(qIdx).exampleRows = exampleRows;   % --- NEW ---
        ...
        plotQueue(qIdx).thisCol = thisCol;
        plotQueue(qIdx).exampleRows = exampleRows;   % --- NEW: same rows for Even ---
    end
    ...
    for iCol = 1:nCols
        ...
        % --- NEW: mark example rows with numbered arrows to the right of the heatmap ---
        if isfield(q, 'exampleRows') && ~isempty(q.exampleRows)
            hold(ax_heat, 'on');
            nBins = size(q.matrixToPlot, 2);
            for e = 1:length(q.exampleRows)
                if ~isnan(q.exampleRows(e))
                    plot(ax_heat, nBins + 3, q.exampleRows(e), 'k>', ...
                        'MarkerSize', 6, 'MarkerFaceColor', 'k', 'Clipping', 'off');
                    text(ax_heat, nBins + 6, q.exampleRows(e), num2str(e), ...
                        'FontSize', 10, 'FontName', targetFont, ...
