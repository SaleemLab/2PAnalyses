% =========================================================================
% This script uses the masterdata table to pick out specific sessions and
% regenerate updated processedTwoPData.mat files to include DF/F and
% DF/F+Neuropil correction. It has changed the F0 calculation to 60s
% (window_size). It also loads the Response.mat file and resaves
% LapPositionActivity using DFF and DFFNeuropilCorrected.
...
disp('Loading and parsing master table...');
masterTable = loadAndParseMasterTable(masterTableFilePath);

disp("Filtering table for 'Boutons' sessions with 'VRCorr' stimulus...");
filteredTable = filterMasterTable(masterTable, ...
    'TypeImaged', 'Boutons', ...
    'HasStimulus', 'VRCorr');
  ...
  %%      
        ...
        if ~isempty(vrCorrStimFiles)
            ...
            for thisVRIdx = 1:length(vrCorrStimFiles)
                ...
                % Use a try-catch block to gracefully handle errors for any stimulus
                try
                    % Rerun neuropil correction and DFF; Append to processedTwoPData
                    % [processedTwoPData, sessionFileInfo] = computeNeuropilCorrectionAndDFF(sessionFileInfo, currentVRFile);
