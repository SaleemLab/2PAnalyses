...
% filteredTable = filterMasterTable('Exclude', 0, 'Suite2PPreprocessing', 1, 'TypeImaged', 'Boutons');
% mouseInfo = sessionsToProcess(filteredTable);
%% SET PROCESSING PARAMETERS:


interpRate = 60;        % Hz, for resampling
frameRate = 7.28;       % Frame rate 
...
for thisMouse = 1:size(mouseInfo, 1)
    ...
    for thisSession = 1:length(sessionNames)
        ...
        try
            ...
            % This now has access to twpPData and 2PMetadata for all stimuli 
            ...
            if ~isempty(vrStimNames)
                ...
                for thisVRStim = 1:length(vrStimNames)
                    ...
                    [~]        = findBonsaiPeripheralLag(sessionFileInfo, 1, 60);
                    [~, sessionFileInfo]        = alignVRBonsaiToPeripheralData(sessionFileInfo,vrStimName);
                    [~, ~, ~, sessionFileInfo]  = resamplAndAlignVR_BonsaiPeripheralSuite2P(sessionFileInfo,60,'TwoPFrameTime', vrStimName, true, true); % Edit this function to save information about ROI
                    [~, sessionFileInfo]        = extractVRAndPeripheralData(sessionFileInfo, vrStimName, true);
                    % Filter out laps to include (TODO) 
                    ...
                    % [response, sessionFileInfo] = getLapPositionActivity(sessionFileInfo, vrStimName, true); OLD
                    [response, sessionFileInfo] = getLapPositionActivity_withShuffle(sessionFileInfo, vrStimName, true); % default is overwrite 
                    % Can also write an alternative shuffle function where
                    % first half and second half suffled and use peak/range
                    
                    % Functions to filter out ROIs: appends this info to sessionROIData
                    [~,~,~,~] = findHightlyCorrelatedROIs(sessionFileInfo); %  VRThreshold < 0.5 and NnVRThreshold < 0.3
                    [~,~] = getRangeSignificance_fromShuffle(sessionFileInfo, response); % > 95th perentile 
                    [~, ~] = getPeakSignificance_fromShuffle(sessionFileInfo, response); % > 95th perentile 
                    [~,~,~]= computeVarianceAcrossPositionBins(sessionFileInfo, response); % TODO have not set a threshold yet % VarToVarTunng < 40 and VarToTuningRange < 0.5
                    [~,~,~] = checkOddEvenCorrelation(sessionFileInfo, response);
                    [~,~,~] = checkHalvesCorrelation(sessionFileInfo, response);

                    
                    % Use time taken to complete lap.. 
                    % Use Odd Even for display..
                    % Use First and last... or do this for the shuffle.. 
                    % Use number of laps



  
                    % [R_sorted, normOdd, normEven, sortIdx, numCells, dataScope, signalToUse] =  
                    % SpatialTuningAndBoutonCorrelationMatrix(mouseID, session, targetArea, checkVRRecOnly, checkNonVRRecOnly, onlyIncludeROIs, signalToUse, applySmoothing, plotFlag)


                

                    % Get running speed across laps 
                    % [response, sessionFileInfo] = getRunningSpeedAcrossLaps(sessionFileInfo, VRStimName, processedTwoPData);
