function crossValExpVar = getCrossValidatedExplainedVariance(sessionFileInfo, VRStimName, response, signalToUse, nFolds, excludeFlaggedLaps, plotFlag)
...
%   signalToUse         - String key for the signal data type
...
if nargin < 6; excludeFlaggedLaps = true; end
if nargin < 7; plotFlag = true; end
...
%% Save results
crossValExpVar.cvExpVar = cvExpVar;
crossValExpVar.medianExpVar = medianExpVar;
crossValExpVar.meanExpVar = meanExpVar;
crossValExpVar.cvExpVarNull = cvExpVarNull; 
crossValExpVar.null95thPercentile = null95thPercentile;
crossValExpVar.null99thPercentile = null99thPercentile;
crossValExpVar.pValues = pValues;
crossValExpVar.nFolds = nFolds;
crossValExpVar.nShuffles = nShuffles;
crossValExpVar.signalUsed = signalToUse;
...
if plotFlag
    ...
    significantCellsIdx = find(crossValExpVar.pValues < 0.01); %& crossValExpVar.meanExpVar > threshold); 
    
    fprintf('Found %d statistically significant, spatially stable cells out of %d total ROIs.\n', ...
        length(significantCellsIdx), length(crossValExpVar.meanExpVar));
        
    % Overall distribution 
    figure('Color', 'w', 'Position', [100, 100, 1100, 400]);
    subplot(1, 4, 1); 
    
    histogram(crossValExpVar.meanExpVar, 'BinWidth', 0.01, ...
        'Normalization', 'probability', ...
        'FaceColor', [0.2 0.5 0.8], 'EdgeColor', 'w', 'FaceAlpha', 0.6);
   
    xlabel('Explained Variance (R^2)');
    ylabel('Probability');
    title(sprintf('Population Distribution\n(%d Stable ROIs (p < 0.01)', length(significantCellsIdx)));
    legend('Real Data (Mean)', 'Location', 'NorthEast');
    set(gca, 'TickDir', 'out', 'box', 'off');
    legend boxoff;
    
    % Example ROIs
    [~, sortedIdx] = sort(crossValExpVar.pValues);
    
    % First, middle and last
    if length(sortedIdx) >= 3
        exampleROIs = [sortedIdx(1), sortedIdx(round(length(sortedIdx)/2)), sortedIdx(end)];
        roiLabels = {'Highly Significant', 'Median p-value', 'Least Significant'};
    else
    ...
    for idx = 1:length(exampleROIs)
        ...
        histogram(crossValExpVar.cvExpVarNull(roiIdx, :), 'BinWidth', 0.01, ...
            'Normalization', 'probability', ...
            'FaceColor', [0.5 0.5 0.5], 'EdgeColor', 'w', 'FaceAlpha', 0.5);
        hold on;
        
        % Vertical line for the 99th percentile confidence limit
        xline(crossValExpVar.null95thPercentile(roiIdx), 'r--', 'LineWidth', 1.5);
        % Vertical line for the 99th percentile confidence limit
        xline(crossValExpVar.null99thPercentile(roiIdx), 'g--', 'LineWidth', 1.5);
        
        % Vertical line for the actual true mean performance
        xline(crossValExpVar.meanExpVar(roiIdx), 'b-', 'LineWidth', 2);
        
        xlabel('R^2');
        title(sprintf('%s (ROI %d)\np = %0.3f', roiLabels{idx}, roiIdx, crossValExpVar.pValues(roiIdx)), 'FontSize', 9);
        set(gca, 'TickDir', 'out', 'box', 'off');
