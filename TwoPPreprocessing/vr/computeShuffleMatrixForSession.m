...
for iStim = 1:length(vrStimNamesToStitch)
    ...
    % Load processedTwoPData for raw signal
    load(sessionFileInfo.stimFiles(stimIdx).processedMergedBonsaiSuite2pData, 'processedTwoPData');
    
    % Select signals based on flag
    signals = processedTwoPData.(iif(useZScoredProcessedSignals, 'zScoredProcessedSignals', 'processedSignals'));
...
clear processedTwoPData currentChunk; 
