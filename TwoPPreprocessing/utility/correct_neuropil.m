%testing sylvia's version 
function [signal, regPars, F_binValues, N_binValues] = correct_neuropil(F, N, numN, minNp, maxNp, prctl_F, prctl_F0, Npil_window_F0)
    % Default parameters
    if nargin < 4, numN = 20; end
    if nargin < 5, minNp = 10; end
    if nargin < 6, maxNp = 90; end
    if nargin < 7, prctl_F = 5; end
    if nargin < 8, prctl_F0 = 8; end % Python default is 8
    if nargin < 9, Npil_window_F0 = 60; end

    [nt, nROIs] = size(F);

    % Get baselines (Using the gaussian smooth version)
    F0 = get_F0(F, prctl_F0, Npil_window_F0);
    N0 = get_F0(N, prctl_F0, Npil_window_F0);
    ...
    for iROI = 1:nROIs
        ...
        % Neuropil range and binning
        N_prct = prctile(iN, [minNp, maxNp]);
        binSize = (N_prct(2) - N_prct(1)) / numN;

        % Exact Python bin values (start of bin)
        N_binValues(:, iROI) = N_prct(1) + (0:numN-1)' * binSize;

        % Assign frames to bins
        N_ind = floor((iN - N_prct(1)) / binSize) + 1;

        % Calculate percentile per bin (matches Python's inner loop)
        for Ni = 1:numN
            mask = (N_ind == Ni);
            if any(mask)
                F_binValues(Ni, iROI) = prctile(iF(mask), prctl_F);
            end
        end

        % Linear Regression (slope b, intercept a)
        valid = ~isnan(F_binValues(:, iROI));
        if sum(valid) > 1
            p = polyfit(N_binValues(valid, iROI), F_binValues(valid, iROI), 1);
            b = p(1); a = p(2);

            % Restrict slope
            % Cap slope at 0.7 to prevent over-subtraction (crushing the signal).
            % A slope > 1 usually indicates contaminated neuropil masks or
            % out-of-focus light (allen white paper 2p) 
            b = min(max(b, 0), 0.7); %change this to 0.75? This 
            regPars(:, iROI) = [a; b];
