function [signal, regPars, F_binValues, N_binValues] = correct_neuropil_OLD(F, N, fs, numN, minNp, maxNp, prctl_F, prctl_F0, Npil_window_F0, verbose)
    ...
    if nargin < 7, prctl_F = 5; end % very close to the min but not the absolute min.. 
    if nargin < 8, prctl_F0 = 5; end
    if nargin < 9, Npil_window_F0 = 180; end % changed from 60 to 180 on 19/09
    ...
    F0 = get_F0(F, fs, prctl_F, Npil_window_F0);
    N0 = get_F0(N, fs, prctl_F, Npil_window_F0);
